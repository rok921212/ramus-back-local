...
const existingUser = await User.findOne({ email }).maxTimeMS(5000);
    ...
    const token = jwt.sign({ userId: user._id }, config.JWT_SECRET, { expiresIn: '12h' });
    res.status(200).json({
      user: {
        _id: user._id,
...
const loginUser = async (req, res) => {
  try {
    ...
    const token = jwt.sign({ userId: user._id }, config.JWT_SECRET, { expiresIn: '12h' });
    console.log(`[users] login ok user=${user._id}`);
    res.status(200).json({
      user: {
        _id: user._id,
        username: user.username,
...
// --- LOGOUT USER ---
// Stateless by design: a JWT can't be server-side invalidated without
// added revocation infrastructure (see index.js's idle second Upstash
// Redis client for where a jti blocklist could live if ever needed) — the
// short (12h) token expiry is the accepted bound on how long a "logged
// out" token can still technically be replayed. The client is responsible
// for discarding its stored token; there's nothing left for the server to
// clear.
